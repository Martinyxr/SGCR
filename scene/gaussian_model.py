...
class GaussianModel:
    ...
    longest_axis_scale = 0.005 # 0.005
    other_axis_ratio = 1    # 1
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        #     np.asarray(pcd.points)).float().cuda()), 0.0000001)     # [N]
        # print("dist2:", dist2.shape)
        # mean_dist = torch.mean(torch.sqrt(dist2))   # [0.016]
        longest_axis_scale = torch.tensor(self.longest_axis_scale).float().cuda()
        scales = torch.log(longest_axis_scale)[..., None].repeat(fused_point_cloud.shape[0], 3)
        scales[:, 1:] = torch.log(self.other_axis_ratio * longest_axis_scale)
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation):
        ...
        # self._scaling = optimizable_tensors["scaling"]
        longest_axis_scale = torch.tensor(self.longest_axis_scale).float().cuda()
        scales = torch.log(longest_axis_scale)[..., None].repeat(self.get_xyz.shape[0], 3)
        scales[:, 1:] = torch.log(self.other_axis_ratio * longest_axis_scale)
        self._scaling = scales
        self._rotation = optimizable_tensors["rotation"]
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, min_color=0.005):
        ...
        shs = self.get_features.transpose(1, 2).view(-1, 3)
        gray = torch.amax(SH2RGB(shs), dim=-1)
        color_mask = (gray >= min_color).squeeze()
        retain_mask = ~prune_mask
        retain_mask = torch.logical_or(retain_mask, color_mask)
        prune_mask = ~retain_mask
    ...
    def final_prune(self, min_opacity=0.5, min_color=0.4, min_scale=0.0005):
        opacity_mask = (self.get_opacity < 0.5).squeeze() # 0.5
        # print("features:", self.get_features.shape)
        shs = self.get_features.transpose(1, 2).view(-1, 3)
        rgb = SH2RGB(shs)
        gray = torch.amax(rgb, dim=-1)
        # print("gray:", gray.shape)
        color_mask = (gray < min_color).squeeze()
        # print("opacity_mask:", opacity_mask.shape, "color_mask:", color_mask.shape)
        # exit(0)

        # prune_mask = torch.logical_or(opacity_mask, color_mask)
        # prune_mask = color_mask
        prune_mask = opacity_mask

        num1 = self.get_gaussian_numbers
        self.prune_points(prune_mask)
        num2 = self.get_gaussian_numbers
        print("Prune before:", num1, "  Prune after:", num2)
